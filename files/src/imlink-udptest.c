...
int main(int argc,char *argv[])
{
    ...
    my_addr.sin_port=htons(18266);
    my_addr.sin_addr.s_addr=inet_addr("192.168.8.255");
    bzero(&(my_addr.sin_zero),8);
    
    user_addr.sin_family=AF_INET;
    user_addr.sin_port=htons(18266);
   ...
   setsockopt(socket_fd,SOL_SOCKET,SO_BROADCAST,&so_broadcast,sizeof(so_broadcast));
    ...
    if((sendto(socket_fd,buf,strlen(buf),0,(struct sockaddr *)&my_addr,sizeof(my_addr))) == -1)
    	perror("sendto");
