 ...
 *
 * Description: Send response message back to User's App to notice the result of imlink.
 */
...
//	}
    ...
    {
        strcpy(buf, "{\"command\":\"hello\"}");
	
        sendto(socket_descriptor,buf,strlen(buf)+1,0,(struct sockaddr *)&address,sizeof(address));
        usleep(1000);
    }
